...
| **C142** | The forced-slack dichotomy is one-sided: the block slack is never arithmetically forced, and the gap slack is forced only when $f_i\le2$; on a pure-upper chain C120's $f_i\ge5$ leaves neither forced | `slack-coordinates.md` | **Corollary - new; raw-trace check confirms 0 forced block slacks and $f_i=1$ in every modulus-dominant gap case; external review pending; redirects the descent target** |
