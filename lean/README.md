...
* **The block-chain length bound (Theorem 145) and its arithmetic core.** These
  live on the `research/admissible-budget` branch. This branch carries the paper
  and the chain the paper cites; the frontier mathematics was moved off so the
  manuscript can be reviewed as one unit.
